...
// Worker URLs - MUST be set via environment variables (no defaults)
const EMBEDDING_WORKER_URL = process.env.QMD_EMBED_WORKER_URL;
const RERANK_WORKER_URL = process.env.QMD_RERANK_WORKER_URL;

if (!EMBEDDING_WORKER_URL || !RERANK_WORKER_URL) {
  throw new Error("CloudLLM requires QMD_EMBED_WORKER_URL and QMD_RERANK_WORKER_URL environment variables");
}
